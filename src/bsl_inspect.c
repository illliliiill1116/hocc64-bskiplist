...
bsl_inspect_all(bsl_t *list, bsl_inspect_result_t *out)
{
    ...
    local.order_ok       = bsl_inspect_order(list);
    ...
    printf("  Leaf nodes   : %lu  Internal: %lu  Keys: %lu  Avg fill: %.1f%%\n",
           local.total_leaves, local.total_internals,
           local.total_keys, local.avg_leaf_fill * 100.0);
    printf("  Histogram    :");
    for (int i = 0; i <= 10; i++)
    {
        if (local.fill_buckets[i] > 0)
            printf("  %d0%%:%d", i, local.fill_buckets[i]);
    }
    printf("\n");
