...
static inline int split_keys(void *src_node, void *dst_node, int start_rank, int dst_offset) 
{
    ...
    STORE_RELAXED(src_h->num_elts, (uint16_t)start_rank);
    __atomic_add_fetch(&dst_h->num_elts, (uint16_t)move_cnt, __ATOMIC_RELAXED);
    return move_cnt;
}

static inline void insert_internal_slot(void *node, int rank, bsl_key_t key, void *child) {
...
static inline void insert_leaf_slot(void *node, int rank, bsl_key_t key, bsl_val_t value) {
    node_header_t *h = (node_header_t *)node;
    bsl_key_t *keys = NODE_KEYS(node);
...
top_retry:;
    ...
    if (found_key)
    {
        ...
        else
        {
            ...
            for (int i = 0; i < level_to_promote; i++)
            {
                if (new_nodes[i]) bsl_free_node(new_nodes[i]);
            }
            return 1;
        } 
    }
    ...
    int max_key = (level == 0) ? B_LEAF : B_INTERNAL;
    if (curr->num_elts + 1 > max_key)
    {
        node_header_t *new_node = bsl_alloc_node();
